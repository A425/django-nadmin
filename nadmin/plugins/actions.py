...
from nadmin.util import model_format_dict, get_deleted_objects, model_ngettext
from nadmin.views import BaseAdminPlugin, ListAdminView
from nadmin.views.base import filter_hook, ModelAdminView
...
class ActionPlugin(BaseAdminPlugin):
    ...
    def get_media(self, media):
        if self.actions and self.admin_view.result_count:
            media = media + self.vendor('nadmin.plugin.actions.js', 'nadmin.plugins.css')
        return media

    # Block Views
    def block_results_bottom(self, context, nodes):
        if self.actions and self.admin_view.result_count:
            nodes.append(loader.render_to_string('nadmin/blocks/model_list.results_bottom.actions.html', context_instance=context))
