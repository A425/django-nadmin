...
from nadmin.layout import FormHelper, Layout, flatatt, Container, Column, Field, Fieldset
from nadmin.sites import site
from nadmin.views import BaseAdminPlugin, ModelFormAdminView, DetailAdminView, filter_hook


class ShowField(Field):
    template = "nadmin/layout/field_value.html"

    def __init__(self, admin_view, *args, **kwargs):
        super(ShowField, self).__init__(*args, **kwargs)
        self.admin_view = admin_view
        if admin_view.style == 'table':
            self.template = "nadmin/layout/field_value_td.html"
...
class TDField(Field):
    template = "nadmin/layout/td-field.html"
...
class InlineStyle(object):
    template = 'nadmin/edit_inline/stacked.html'
...
class InlineFormset(Fieldset):
    ...
    def __init__(self, formset, allow_blank=False, **kwargs):
        ...
        if allow_blank and len(formset) == 0:
            self.template = 'nadmin/edit_inline/blank.html'
    ...
    def get_media(self, media):
        ...
        if self.formsets:
            media = media + self.vendor(
                'nadmin.plugin.formset.js', 'nadmin.plugin.formset.css')
        return media
