...
from nadmin.layout import FormHelper, Layout, Fieldset, Container, Col
from nadmin.plugins.actions import BaseActionView, ACTION_CHECKBOX_NAME
from nadmin.util import model_ngettext, vendor
from nadmin.views.base import filter_hook
from nadmin.views.edit import ModelFormAdminView
...
class ChangeFieldWidgetWrapper(forms.Widget):
    ...
    @property
    def media(self):
        media = self.widget.media + vendor('nadmin.plugin.batch.js')
        return media
...
class BatchChangeAction(BaseActionView):
    ...
    def get_media(self):
        ...
        media = media + self.form_obj.media + self.vendor(
            'nadmin.page.form.js', 'nadmin.form.css')
        return media
