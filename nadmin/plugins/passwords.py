...
from nadmin.views.base import BaseAdminPlugin, BaseAdminView, csrf_protect_m
from nadmin.views.website import LoginView
...
class ResetPasswordSendView(BaseAdminView):
    ...
    password_reset_template = 'nadmin/auth/password_reset/form.html'
    password_reset_done_template = 'nadmin/auth/password_reset/done.html'
    password_reset_token_generator = default_token_generator

    password_reset_from_email = None
    password_reset_email_template = 'nadmin/auth/password_reset/email.html'
...
class ResetPasswordComfirmView(BaseAdminView):
    ...
    password_reset_confirm_template = 'nadmin/auth/password_reset/confirm.html'
    password_reset_token_generator = default_token_generator
    ...
    def get_media(self):
        return super(ResetPasswordComfirmView, self).get_media() + \
            self.vendor('nadmin.page.form.js', 'nadmin.form.css')

site.register_view(r'^nadmin/password_reset/(?P<uidb36>[0-9A-Za-z]{1,13})-(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
    ResetPasswordComfirmView, name='nadmin_password_reset_confirm')
...
class ResetPasswordCompleteView(BaseAdminView):
    ...
    password_reset_complete_template = 'nadmin/auth/password_reset/complete.html'
...
site.register_view(r'^nadmin/password_reset/complete/$', ResetPasswordCompleteView, name='nadmin_password_reset_complete')
