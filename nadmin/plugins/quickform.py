...
from nadmin.util import get_model_from_relation, vendor
from nadmin.views import BaseAdminPlugin, ModelFormAdminView
from nadmin.layout import Layout
...
class QuickFormPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        if self.request.method == 'GET' and self.request.is_ajax() or self.request.GET.get('_ajax'):
            self.admin_view.add_form_template = 'nadmin/views/quick_form.html'
            self.admin_view.change_form_template = 'nadmin/views/quick_form.html'
            return True
        return False
...
class RelatedFieldWidgetWrapper(forms.Widget):
    ...
    @property
    def media(self):
        media = self.widget.media + vendor('nadmin.plugin.quick-form.js')
        return media
