...
from nadmin.plugins.ajax import JsonErrorDict
from nadmin.sites import site
from nadmin.util import lookup_field, display_for_field, label_for_field, unquote, boolean_icon
from nadmin.views import BaseAdminPlugin, ModelFormAdminView, ListAdminView
from nadmin.views.base import csrf_protect_m, filter_hook
from nadmin.views.edit import ModelFormAdminUtil
from nadmin.views.list import EMPTY_CHANGELIST_VALUE
    ...
    def get_media(self, media):
        if self.editable_need_fields:
            media = media + self.model_form.media + \
                self.vendor(
                    'nadmin.plugin.editable.js', 'nadmin.widget.editable.css')
        return media
